...
#ifdef PADDLE_WITH_CUDA
template <typename T>
class LayerNormV2DirectCUDAFunctor {
 public:
...
template <typename DeviceContext, typename T>
class LayerNormV2Kernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    ...
    auto ker =
        jit::KernelFuncs<jit::LayerNormV2Tuple<T>, platform::CPUPlace>::Cache()
            .At(right);
...
template <typename DeviceContext, typename T>
class LayerNormV2GradKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext& ctx) const override {
    const float epsilon = ctx.Attr<float>("epsilon");
    auto x = *ctx.Input<Tensor>("X");
