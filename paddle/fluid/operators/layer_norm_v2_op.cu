...
template <typename T>
using LayerNormV2ParamType = typename CudnnDataType<T>::BatchNormParamType;
...
template <typename T>
struct PairForLayerNormV2 {
  __device__ __forceinline__ PairForLayerNormV2() {}
  __device__ __forceinline__ PairForLayerNormV2(const T &first, const T &second)
...
template <typename T>
struct PairForLayerNormV2AddFunctor {
  __device__ __forceinline__ PairForLayerNormV2<T> operator()(
      const PairForLayerNormV2<T> &p1, const PairForLayerNormV2<T> &p2) {
    return PairForLayerNormV2<T>(p1.first_ + p2.first_,
...
__global__ void LayerNormV2Forward(const T *x, const U *scale, const U *bias,
                                   T *y, U *mean, U *var, float epsilon,
                                   int feature_size) {
  using BlockReduce = cub::BlockReduce<PairForLayerNormV2<U>, BlockDim>;
  ...
  auto pair = BlockReduce(temp_storage)
                  .Reduce(PairForLayerNormV2<U>(mean_val, var_val),
                          PairForLayerNormV2AddFunctor<U>());
  if (threadIdx.x == 0) {
...
__inline__ __device__ void cuLoadAddStridedInputs(
    ...
    const T *output, const T *dout, const int i1_end, const int n2,
...
__global__ void LayerNormV2BackwardPartGradGammaBeta(
    const T *__restrict__ dout, const T *__restrict__ output, const int n1,
    const int n2, const U *__restrict__ scale, const U *__restrict__ bias,
  ...
  for (int i1_block = blockIdx.y * BDIMY * VPTX; i1_block < n1;
    ...
    cuLoadAddStridedInputs<T, U, VPTX>(
        ...
        warp_buf1, warp_buf2, output, dout, n1, n2, scale, bias);
  }
  __syncthreads();
...
__global__ void LayerNormV2BackwardSumGradGammaBeta(
...
template <typename T, typename U, int BDIMX, int BDIMY>
__global__ void LayerNormV2BackwardComputeGradInput(
    const T *__restrict__ dout, const T *__restrict__ output, const int n1,
    const int n2,
    // const U* __restrict__ mean, const U* __restrict__ var, const float
    // epsilon, const T* gamma,
    const U *__restrict__ var, const float epsilon, const U *gamma,
    const U *bias, T *grad_input) {
...
__global__ void LayerNormV2BackwardGradientAll(const T *y, const T *d_y,
                                               U *d_scale, U *d_bias, T *d_x,
                                               const U *var, const U *scale,
                                               const U *bias, float epsilon,
  ...
  for (int i = beg_idx; i < end_idx; i += stride) {
    ...
    d_scale_partial += static_cast<U>(d_y[i]) *
                       (static_cast<U>(y[i]) - bias[row_idx]) / scale[row_idx];
  ...
  auto pair =
      BlockReduce(temp_storage)
          .Reduce(PairForLayerNormV2<U>(d_scale_partial, d_bias_partial),
                  PairForLayerNormV2AddFunctor<U>());
...
__global__ void LayerNormV2BackwardGradientScaleOrBias(
...
template <typename T, typename U, int BlockDim>
__global__ void LayerNormV2BackwardPostProcessToCalculateDX(const T *x, T *d_x,
                                                            const U *var,
                                                            float epsilon,
                                                            int feature_size) {
  using BlockReduce = cub::BlockReduce<PairForLayerNormV2<U>, BlockDim>;
  ...
  auto pair =
      BlockReduce(temp_storage)
          .Reduce(PairForLayerNormV2<U>(d_x_mean_partial, d_x_var_partial),
                  PairForLayerNormV2AddFunctor<U>());
...
template <typename T, typename U, int BlockDim>
__global__ void LayerNormV2BackwardGradientOnlyDX(const T *x, const T *d_y,
                                                  T *d_x, const U *mean,
                                                  const U *var, const U *scale,
                                                  float epsilon,
                                                  int feature_size) {
...
__global__ void LayerNormV2BackwardWhenBatchSizeIsOne(
...
template <typename T, typename U>
static void LayerNormV2Backward(const T *y, const T *d_y, const U *scale,
                                const U *bias, const U *var, T *d_x, U *d_scale,
  ...
  if (batch_size == 1) {
    LayerNormV2BackwardWhenBatchSizeIsOne<
        T, U><<<(feature_size + kMaxBlockDim - 1) / kMaxBlockDim, kMaxBlockDim,
                0, stream>>>(y, d_y, d_x, d_scale, d_bias, var, scale, bias,
                             epsilon, feature_size);

    if (d_x != nullptr) {
      switch (GetDesiredBlockDim(feature_size)) {
        // TODO(ZHUI): fix bias
        FIXED_BLOCK_DIM_CASE(LayerNormV2BackwardPostProcessToCalculateDX<
                             T, U, kBlockDim><<<1, kBlockDim, 0, stream>>>(
            y, d_x, var, epsilon, feature_size));
      }
    }
    return;
  }
  ...
  switch (gradient_flag) {
    case 1:  // d_x == nulptr, d_scale == nullptr, d_bias != nullptr
      switch (block_dim) {
        FIXED_BLOCK_DIM_FIXED_BLOCK_NUM_CASE(
            ...
            LayerNormV2BackwardGradientScaleOrBias<
                T, U, kBlockDim, false,
                false><<<block_num, kBlockDim, 0, stream>>>(
                y, d_y, d_scale, d_bias, d_x, var, scale, bias, epsilon,
    ...
    case 3:  // d_x == nullptr, d_scale != nulptr, d_bias != nullptr
      switch (block_dim) {
        FIXED_BLOCK_DIM_FIXED_BLOCK_NUM_CASE(
            feature_size, kMaxBlockNum,
            LayerNormV2BackwardGradientAll<
    ...
    case 4:  // d_x != nullptr, d_scale == nullptr, d_bias == nullptr
      switch (GetDesiredBlockDim(feature_size)) {
        FIXED_BLOCK_DIM_CASE(
            LayerNormV2BackwardGradientOnlyDX<
                T, U, kBlockDim><<<batch_size, kBlockDim, 0, stream>>>(
                y, d_y, d_x, var, scale, bias, epsilon, feature_size));
      }
      break;
    case 5:  // d_x != nulptr, d_scale == nullptr, d_bias != nullptr
      ...
      switch (GetDesiredBlockDim(feature_size)) {
        FIXED_BLOCK_DIM_CASE(
            // TODO(ZHUI): fix y bias
            LayerNormV2BackwardPostProcessToCalculateDX<
                T, U, kBlockDim><<<batch_size, kBlockDim, 0, stream>>>(
                y, d_x, var, epsilon, feature_size));
      }
      break;
    ...
    case 7:  // d_x != nullptr, d_scale != nullptr, d_bias != nullptr
    {
      ...
      LayerNormV2BackwardPartGradGammaBeta<
          T, U, BDIMX2, BDIMY2, VPT><<<blocks2, threads2, 0, stream>>>(
          d_y, y, batch_size, feature_size, scale, bias, part_grad_gamma,
      ...
      LayerNormV2BackwardSumGradGammaBeta<
      ...
      LayerNormV2BackwardComputeGradInput<
          T, U, BDIMX1, BDIMY1><<<blocks1, threads1, 0, stream>>>(
          d_y, y, batch_size, feature_size, var, epsilon, scale, bias, d_x);
      break;
    }
    default:
      break;
...
template <typename T>
void LayerNormV2DirectCUDAFunctor<T>::operator()(
    cudaStream_t stream, const T *input, std::vector<int> input_shape,
    const T *bias, const T *scale, T *output, T *mean, T *variance,
  ...
  switch (GetDesiredBlockDim(feature_size)) {
    FIXED_BLOCK_DIM_CASE(
        LayerNormV2Forward<T, T,
                           kBlockDim><<<batch_size, kBlockDim, 0, stream>>>(
            input, scale, bias, output, mean, variance, eps, feature_size));
    default:
      PADDLE_THROW(platform::errors::InvalidArgument(
          "Product from begin_norm_axis to end in layer_norm_v2 must be larger "
          "than 1"));
      break;
  }
}
...
template <typename T>
class LayerNormV2Kernel<platform::CUDADeviceContext, T>
    : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    using U = LayerNormV2ParamType<T>;
...
template <typename T>
class LayerNormV2GradKernel<platform::CUDADeviceContext, T>
    : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    using U = LayerNormV2ParamType<T>;
...
template class LayerNormV2DirectCUDAFunctor<float>;
...
REGISTER_OP_CUDA_KERNEL(
    layer_norm_v2,
    ops::LayerNormV2Kernel<paddle::platform::CUDADeviceContext, float>,
    ops::LayerNormV2Kernel<paddle::platform::CUDADeviceContext, double>,
    ops::LayerNormV2Kernel<paddle::platform::CUDADeviceContext, plat::float16>);
REGISTER_OP_CUDA_KERNEL(
    layer_norm_v2_grad,
    ops::LayerNormV2GradKernel<paddle::platform::CUDADeviceContext, float>,
    ops::LayerNormV2GradKernel<paddle::platform::CUDADeviceContext, double>,
    ops::LayerNormV2GradKernel<paddle::platform::CUDADeviceContext,
                               plat::float16>);
